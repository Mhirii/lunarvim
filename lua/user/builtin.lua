...
M.config = function()
  ...
  local alpha_opts = require("user.configs.dashboard").config()
  ...
  if lvim.builtin.indentlines.mine then
    require("user.configs.indent_blankline").setup()
  elseif lvim.builtin.indentlines.active then
    require("user.configs.indent_blankline").config()
  end
  ...
  lvim.lsp.buffer_mappings.normal_mode["gp"] = {
    function()
      require("user.configs.peek").Peek "definition"
    end,
    "Peek definition",
  }
  ...
  lvim.builtin.terminal.on_config_done = function()
    M.create_terminal(1, "<A-1>", 20, "float")
  ...
  local languages = vim.tbl_flatten {
    --  "swift", "zig", "query", "r", "scala","solidity", "c_sharp", "d", "dart"
    --  "elixir", "elm", "erlang", "fennel", "hcl", "vimdoc", "java", "jsdoc" 
    -- "julia", "kotlin", "latex", "ledger" "rego", "ruby", "teal", 
    -- { "markdown", "markdown_inline", "nix", "ocaml", "perl", "php", "python" },
    { "bash", "c",  "cmake", "comment", "cpp", "css" },
    { "dockerfile", "fish", "go", "gomod" },
    { "gomod", "graphql", "javascript","html"},
    { "json", "jsonc", "lua", "make" },
    { "regex",  "rust", "scss", },
  ...
  local user_telescope = require "user.configs.telescope"
