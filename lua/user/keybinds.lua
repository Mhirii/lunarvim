...
local function set_comment_box_keymaps()
	lvim.builtin.which_key.mappings["c"] = {
		name = "Comment Box",
		["b"] = { "<Cmd>CBccbox<CR>", "Box Comment" },
		["t"] = { "<Cmd>CBllline<CR>", "Title Comment" },
		["l"] = { "<Cmd>CBline<CR>", "Line Comment" },
		["m"] = { "<Cmd>CBllbox12<CR>", "Mark Box Comment" },
		["d"] = { "<Cmd>CBd<CR>", "Remove Comment" },
	}
...
M.split_window = function()
	lvim.builtin.which_key.mappings["|"] = { "<C-W>v", "Split window right" }
	lvim.builtin.which_key.mappings["-"] = { "<C-W>s", "Split window below" }
end
...
local function set_bufferline_keymaps()
	lvim.keys.normal_mode["<S-x>"] = "<Cmd>lua require('user.configs.bufferline').delete_buffer()<CR>"
	...
	lvim.builtin.which_key.mappings["x"] =
		{ "<Cmd>lua require('user.configs.bufferline').delete_buffer()<CR>", "Kill Buffer" }
...
M.config = function()
	...
	if lvim.builtin.comment_box.active then
		set_comment_box_keymaps()
	end
	...
	lvim.keys.visual_mode["<leader>st"] = "<Cmd>lua require('user.configs.telescope').grep_string_visual()<CR>"
	...
	lvim.builtin.which_key.mappings["F"] = {
		name = " Find",
		b = { "<cmd>lua require('user.configs.telescope').builtin()<cr>", "Builtin" },
		f = { "<cmd>lua require('user.configs.telescope').curbuf()<cr>", "Current Buffer" },
		g = { "<cmd>lua require('user.configs.telescope').git_files()<cr>", "Git Files" },
		i = { "<cmd>lua require('user.configs.telescope').installed_plugins()<cr>", "Installed Plugins" },
		l = {
			"<cmd>lua require('telescope.builtin').resume()<cr>",
			"Last Search",
		},
		p = { "<cmd>lua require('user.configs.telescope').project_search()<cr>", "Project" },
		s = { "<cmd>lua require('user.configs.telescope').git_status()<cr>", "Git Status" },
		z = { "<cmd>lua require('user.configs.telescope').search_only_certain_files()<cr>", "Certain Filetype" },
	}
	...
	if vim.fn.has("nvim-0.10") == 1 then
		lvim.builtin.which_key.mappings["I"] =
			{ "<cmd>lua require('user.neovim').inlay_hints()<cr>", " Toggle Inlay" }
	end
	lvim.builtin.which_key.mappings.l.name = " LSP"
	lvim.builtin.which_key.mappings["f"] = {
		require("user.configs.telescope").find_project_files,
		" Find File",
	}
	...
	lvim.builtin.which_key.mappings["lp"] = {
		name = "Peek",
		d = { "<cmd>lua require('user.configs.peek').Peek('definition')<cr>", "Definition" },
		t = { "<cmd>lua require('user.configs.peek').Peek('typeDefinition')<cr>", "Type Definition" },
		i = { "<cmd>lua require('user.configs.peek').Peek('implementation')<cr>", "Implementation" },
	}
	...
	if lvim.builtin.test_runner.active then
		if lvim.builtin.test_runner.runner == "neotest" then
			lvim.builtin.which_key.mappings["t"] = {
				name = "󰙨 Test",
				f = {
					"<cmd>lua require('neotest').run.run({vim.fn.expand('%'), env=require('user.configs.ntest').get_env()})<cr>",
					"File",
				},
				o = { "<cmd>lua require('neotest').output.open({ enter = true, short = false })<cr>", "Output" },
				r = { "<cmd>lua require('neotest').run.run({env=require('user.configs.ntest').get_env()})<cr>", "Run" },
				a = { "<cmd>lua require('user.configs.ntest').run_all()<cr>", "Run All" },
				c = { "<cmd>lua require('user.configs.ntest').cancel()<cr>", "Cancel" },
				R = { "<cmd>lua require('user.configs.ntest').run_file_sync()<cr>", "Run Async" },
